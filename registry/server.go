...
func (r *registry) add(item Registration) error {
	...
	r.mutex.Unlock()
	err := r.sendRequiredServices(item)
	r.notify(patch{
		Added: []patchEntry{
			{
...
func (r registry) sendRequiredServices(item Registration) error {
	r.mutex.RLock()
	defer r.mutex.RUnlock()

	var p patch
	for _, serviceReg := range r.registrations {
		for _, reqService := range item.RequiredServices {
	...
	err := r.sendPath(p, item.ServiceUpdateURL)
	if err != nil {
		return err
	}
	return nil
